...
export class RxScanner {
  ...
  readRxMethods() {
    ...
    addons.concat(classes).forEach((file: string) => {
  ...
  createRxMethodVisit = (checker: ts.TypeChecker) => (node: ts.Node) => {
    switch (node.kind) {
      case ts.SyntaxKind.ImportDeclaration:
        const importDeclaration = node as ts.ImportDeclaration;
        if ((importDeclaration.moduleSpecifier as ts.StringLiteral).text === "rxjs") {
          (importDeclaration.importClause!.namedBindings as ts.NamedImports).elements.forEach(
